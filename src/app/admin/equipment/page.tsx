...
export default async function AdminEquipmentPage() {
  ...
  return <main className="admin-wrap stack" style={{ gap: 28 }}><header><p className="eyebrow">Inventory</p><h1 className="page-title">장비 관리</h1><p className="page-copy">고유 QR이 연결된 사내 장비와 현재 책임자를 관리합니다.</p></header>
    ...
    <section className="data-table-wrap"><table className="data-table"><thead><tr><th>자산번호</th><th>장비명</th><th>상태</th><th>현재 사용자</th><th>작업</th></tr></thead><tbody>{equipment.map(item => { const status = getEffectiveStatus({ operationalStatus: item.operationalStatus, hasActiveAssignment: !!item.assignments.length, hasValidTicket: !!item.transferTickets.length }); return <tr key={item.id}><td className="font-data">{item.assetNumber}</td><td>{item.name}</td><td><StatusBadge status={status}/></td><td>{item.assignments[0]?.user.name ?? "—"}</td><td><Link className="text-link" href={`/admin/equipment/${item.assetNumber}`}>관리·QR</Link></td></tr>; })}</tbody></table></section>
  </main>;
}
