...
export default async function AdminEquipmentPage({ searchParams }: { searchParams: Promise<{ q?: string; page?: string }> }) {
  ...
  return <main className="admin-wrap stack" style={{ gap: 28 }}>
    ...
    <section className="data-table-wrap"><table className="data-table"><thead><tr><th>자산번호</th><th>장비명</th><th>상태</th><th>현재 사용자</th><th>작업</th></tr></thead><tbody>{equipment.map(item => {
      const status = getEffectiveStatus({ operationalStatus: item.operationalStatus, hasActiveAssignment: !!item.assignments.length, hasValidTicket: !!item.transferTickets.length });
      return <tr key={item.id}><td className="font-data">{item.assetNumber}</td><td>{item.name}</td><td><StatusBadge status={status}/></td><td>{item.assignments[0]?.user.name ?? "—"}</td><td><Link className="text-link" href={adminEquipmentPath(item.assetNumber)}>관리·QR</Link></td></tr>;
    })}</tbody></table>{equipment.length === 0 ? <div className="empty-state">조건에 맞는 장비가 없습니다.</div> : null}</section>
