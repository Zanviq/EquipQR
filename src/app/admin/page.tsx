...
import { adminEquipmentPath, adminUserPath } from "@/lib/admin-paths";
...
export default async function AdminDashboardPage() {
  ...
  return <main className="admin-wrap stack" style={{ gap: 28 }}>
    ...
    <div className="admin-columns">
      ...
      <section className="stack"><h2 className="section-title">30일 이상 대여</h2><div className="card">{data.longLoans.length ? data.longLoans.map(item => <Link className="loan-row" key={item.id} href={adminEquipmentPath(item.equipment.assetNumber)}><div><strong>{item.equipment.name}</strong><span className="font-data">{item.equipment.assetNumber}</span></div><span>{item.user.name}</span></Link>) : <div className="empty-state">장기 대여 장비가 없습니다.</div>}</div></section>
    </div>
    <section className="stack"><h2 className="section-title">사용자별 보유 현황</h2><div className="card">{data.userHoldings.length ? data.userHoldings.map(user => <Link className="loan-row" key={user.id} href={adminUserPath(user.employeeNumber)}><div><strong>{user.name}</strong><span className="font-data">{user.employeeNumber}</span></div><span>{user._count.assignments}대{user.status === "INACTIVE" ? " · 조치 필요" : ""}</span></Link>) : <div className="empty-state">장비를 보유한 사용자가 없습니다.</div>}</div></section>
  </main>;
}
